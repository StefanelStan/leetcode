...
public class NextGreaterElementI {
    ...
    public int[] nextGreaterElement(int[] nums1, int[] nums2) {
        int[] greaterElements = getNextGreaterElements(nums2);
    ...
    private int[] getNextGreaterElements(int[] nums) {
        ...
        Deque<Integer> biggerElements = new LinkedList<>();
        for (int i = 0; i < nums.length -1; i++) {
            if(nums[i] < nums[i+1]) {
                greaterElements[nums[i]] = nums[i + 1];
                while(!biggerElements.isEmpty() && nums[biggerElements.peekLast()] < nums[i + 1]){
                    greaterElements[nums[biggerElements.pollLast()]] = nums[i + 1];
                }
            } else {
                biggerElements.addLast(i);
            }
        }
