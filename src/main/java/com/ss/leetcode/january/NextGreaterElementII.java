...
public class NextGreaterElementII {
    ...
    private int getNextGreaterElement(int num, int numIndex, List<Group> groups, int groupSize, int[] nums) {
        ...
        if (groups.get(currentGroupIndex).getMaxValue() > num) {
            nextGreaterElement = getNextGreaterElementFromWithinGroup(groups.get(currentGroupIndex), num, numIndex % groupSize, nums);
        }
        if (nextGreaterElement == Integer.MIN_VALUE && currentGroupIndex + 1 <= groups.size() -1) {
            // no max found here; look right
            nextGreaterElement = searchGroupsForGreaterElement(num, 0, Math.min(currentGroupIndex + 1, groups.size() - 1), groups.size() - 1, groups, nums);
        }
        if (nextGreaterElement == Integer.MIN_VALUE) {
            // no max found here; look left
            nextGreaterElement = searchGroupsForGreaterElement(num, 0, 0, currentGroupIndex, groups, nums);
        }
        return (nextGreaterElement == Integer.MIN_VALUE ? -1 : nextGreaterElement);
    }

    private int searchGroupsForGreaterElement(int num, int numIndex, int startGroupPosition, int endGroupPosition, List<Group> groups, int[] nums) {
    ...
    private int getNextGreaterElementFromWithinGroup(Group group, int num, int numIndex, int[] nums) {
